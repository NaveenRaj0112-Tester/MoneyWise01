...
let statusMonth = new Date().getMonth()+1; // shared Status/Dashboard period filter (0 = All Months)
let statusYear  = new Date().getFullYear(); // 0 = All Years
...
async function logout() {
  ...
  aiCloseOverlay();
  page = 'login'; authPage = 'login'; render();
}
...
function buildDashboard(){
  ...
  eb.addEventListener('click',()=>{page='add-expense';render();});
  const ib=h('button',{class:'btn btn-p'},h('i',{class:'fas fa-plus-circle'}),' Add Income');
  ib.addEventListener('click',()=>{page='add-income';render();});
  ...
  viewAllLink.addEventListener('click',()=>{ page='tx-history'; render(); });
...
function buildAddTx(type){
  ...
  expBtn.addEventListener('click',()=>{page='add-expense';render();});
  const incBtn=h('button',{class:type==='income'?'act':''},'Income');
  incBtn.addEventListener('click',()=>{page='add-income';render();});
...
function buildTransactionHistory(){
  ...
  const phdr=h('div',{class:'page-hdr'});
  const bb=h('button',{class:'back-btn'},h('i',{class:'fas fa-chevron-left'}));
  bb.addEventListener('click',()=>{ page='dashboard'; render(); });
  phdr.appendChild(bb);
...
function buildSettings(){
  ...
  accCard.appendChild(settingsRow('fa-user', 'sri-purple', 'My Profile', state.user.name+' · '+state.user.email, ()=>{ profileTab='profile'; page='profile'; render(); }));
  ...
  accCard.appendChild(settingsRow('fa-clock-rotate-left', 'sri-orange', 'Transaction History', 'Search & filter all your transactions', ()=>{ page='tx-history'; render(); }));
  accCard.appendChild(settingsRow('fa-key', 'sri-green', 'Change Password', 'Update your password', ()=>{ profileTab='password'; page='profile'; render(); }));
...
function showSalaryModal(emp){
  ...
  saveBtn.addEventListener('click',async ()=>{
    ...
    try {
      ...
      overlay.remove();
      await refreshAll();
      page='settings'; settingsPage='salary-management'; render();
